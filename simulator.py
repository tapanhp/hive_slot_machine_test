...
for user in range(1, 2):
    ...
    print(f"Provided initial credit {credit_manager.credits} to user {user}")
    ...
    while True:
        try:
            ...
            print(f"User {user} lost {CREDIT_CUT_FOR_SINGLE_ROLL}, Now credit {credit_manager.credits}")
            ...
            if slot_machine.check_if_jackpot():
                ...
                if should_roll_again(credit_manager.credits):
                    ...
                    # Pull lever should be called here, Right now I am ignoring it
                    continue

                total_jackpots += 1
                total_won_credit_by_user += won_credit
                print(f"User {user} won Jackpot! with credit {won_credit}, previous credits {credit_manager.credits}")
            ...
            print(f"User {user} has exhausted credit, continue with next")
            break
